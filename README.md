# HFMD_long-term-forecasting
本项目引入TSAUG数据增强策略，探索小样本条件下提升长期预测精度的有效途径，可在其他区域开展验证，评估模型在不同流行模式下的适用性。此外，针对长期预测的应用场景，本项目构建趋势准确率、累积相对误差、峰值误差、拐点延迟等专项评估指标，以全面衡量模型在高发季预警和趋势研判中的实用价值。项目提供了完整的训练代码、预训练模型及轻量级预测API，可直接使用。
...
1. 准备一份你的本区域手足口病发病率月度数据，您要预测未来三个月，请至少包含预测步+1的数据，即至少有近期4个月的数据。为了更好评估模型性能，推荐要有预测步+6个月以上的历史数据。文件类型需为csv格式，且列名需为"date"和"rate",将文件放在use_model文件夹下。
...
### 评估
如果你需要DM检验和长期预测的评估，你需要单独调用src/evaluation/下的对应代码文件，且建议将你的源数据也放在该文件夹下。源数据需包含你的真实值，模型的预测值，可以同时比对多个模型。
```csv
y_ture,	LightGBM,CNN-BiLSTM,LSTM,NBEATS,ARIMA,Prophet
0.25, 0.36,0.47,0.42,0.33,0.28   

```
## 论文信息

> 韦国清，李存仙，段云权等. 基于数据增强的手足口病长期预测多模型比较与跨区域验证[J]. 杂志，日期（审稿中）.
